...
### Added
- **The pixel/dither poster capability**, `references/pixel-dither-posters.md`. A poster is not a resized slide: its own A4 grid (2480 × 3508px at 300dpi, the same 20px module, 6 columns × 340px with 40px gutters on a 2240px live width), one fixed reusable template (only field placement, headline and one accent marker vary), a field zone and type zone fixed by the template, and the EN/AR mirror for six columns (`x' = 2480 − x − w`, C1↔C6 / C2↔C5 / C3↔C4). Two construction rules are new: the density-sampling grid extends 20–50% past each poster edge it touches before density is evaluated, so the visible edge does not read as capped; and every field module is generated individually against the density function, never approximated with a blurred gradient. Four example compositions are given as specs
- **`references/inspiration-sessions.md`**: when to run an online inspiration session (before any design no archetype, slide or reference covers; always for a poster), checking the held pool first, query scope, primary-source discipline, palette discipline (take composition from a reference, never its colour), the session-log format, and how a decision goes back through the release checklist
- **`references/approved-inspirations.md`**, the ledger the sessions feed: Approved (four ideas that became poster rules, each with source, credit and the rule it became), Held (four ideas kept for later, each with the rule line it would become) and Rejected (empty; rejection is explicit only). It also lists, by name only, the nine layout patterns adopted from the deck audit. `tests/test_inspiration_ledger.py` checks that every row is sourced and dated, that approved rules point at a real file, that the three states are disjoint and that nothing is rejected by default
- **Eight layout archetypes, 15–22**, in `references/layout-archetypes.md` §1, with construction notes in a new §1.1: Framed poster cover, Stacked word column, Statement slide, Card grid, Detail spread, Metric card panel, Activity matrix and Notice slide. Plus the **Quiet divider**, a zero-motif variant of archetype 2. Deck values they cite as `[M]` were measured by pixel-sampling a render of the deck, not from the Figma file, and the file says so
- **Slide-library entries N1–N11** in `references/slide-library.md`, one per new archetype (two for Card grid, a separate closer for the framed cover) plus the Quiet divider, and a new **Notice and closing** category. They are numbered N because none is built in Figma yet; the plain `#` column stays the Figma frame number
- **Display 280 and 120.** The seven-size slide ladder becomes nine (`editorial-technique.md` §2.10.1). Both take LH ×0.90 and −2.5% tracking under the existing ≥100px rules. 280 is the new ceiling, carrying the framed cover's headline and the cropped over-scale numeral (the ladder is now closed at 280, not 240). 120 is reserved for the Statement slide archetype and does not fill the 60→160 chasm on ordinary slides. Scale ratios, trap pairings, tracking, block heights (both copies of the table), optical offsets, `SKILL.md`'s scale and roles tables, and pass-gate check #6 are updated. The Figma file does not have them yet: `token-system.md` §7 records them as specified 2026-09-24 and pending in `04 Typography`, and the collection counts (504 in total, 41 in `04 Typography`) are unchanged. The style names `Display/2XL` and `Display/S+` are proposals
- **Open client questions Q-1 to Q-9**, `references/decision-law.md`. Q-1 to Q-8 come from the deck audit: Electric behind body text on a dense slide, Electric chips, icons and pills on light grounds, a per-topic section-colour ground family, a pale pink severity chip and its caption ink, near-black `#0D121C` ink, a framed cover inside the bleed-safe margin with the logo as cover art, a six-column matrix grid, and the running-eyebrow and divider anchors. Current rules stand until each is ruled, and every new archetype and N entry that touches one cites it
- **Example 8**, `EXAMPLES.md`: a dither A4 poster request, answered with the grid, the field construction, the type zone and the template rule. The intro now counts eight requests
- **Rendered example slides** (commit `e988b7b`): six 1920×1080 slides in `assets/examples/` (cover, big number, claim and evidence, severity list, divider, Arabic mirror), their HTML sources and `motif.js` dither engine in `assets/examples/src/`, a README strip, an image gallery in `EXAMPLES.md`, and re-render instructions in `CONTRIBUTING.md`

### Changed
- **Q-9 opened: motif coverage on content slides.** `SKILL.md` says 0%, `layout-archetypes.md` §4 says ≤20% of canvas, and the 36-slide library measures a median coverage of 0.027. Neither text is ruled wrong. Both are marked "(open: decision-law Q-9)": `SKILL.md`'s Coverage row, and `layout-archetypes.md`'s Coverage ceiling row, Discipline row and §4.5 content-slide row. `EXAMPLES.md` example 7 points at Q-9 too. The 4.2.3 README correction ("Content slides take 0%") is now under Q-9 rather than settled
- `README.md`: the Motif line now reads "kept to the edges and never under text", which holds under either answer to Q-9. Layout count 14 → 22
- `README.md` cut to the essentials, 167 → 73 lines (commit `fec82b1`): install, the governing rule, what the skill covers, and links. The palette table and the full repository map are dropped; they duplicated `SKILL.md` and the references
- `SKILL.md`: the reference index points at `pixel-dither-posters.md`, and says an inspiration session runs before a poster or any uncovered design, checking `approved-inspirations.md`'s held pool first. It also names the N entries in the slide library and the Q questions in the decision law. The frontmatter description now triggers on "pixel poster", "dither poster" and "A4 poster" and counts 22 archetypes; two connective words were cut to keep it at 1,017 of 1,024 characters
- Archetype counts 14 → 22 in `SKILL.md`, `README.md`, `.claude-plugin/plugin.json` and `references/inspiration-sessions.md`
- `CONTRIBUTING.md`: "Where new knowledge goes" gains rows for posters, inspiration sessions and the inspiration ledger

### Corrected
- **Primary claim leading, `editorial-technique.md` §2.10.1: ×1.16 → ×1.35.** The ladder row for the 40px primary claim still read 1.16, the body leading that 3.0.0 superseded with ×1.35. The same file's §2.4 ("Body 12–40, ×1.35, ratified") and its ×1.35 ratification table already gave 54px at 40. No other live 1.16 remains; the rest are in superseded or historical context
