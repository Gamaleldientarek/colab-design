...
The skill disagrees with itself here too (found 2026-09-24 while building `assets/tokens/`): `token-system.md` §5.4, which documents the live Figma file, sets `accent/default` in **Light** mode to `#34FF67`, and `chip/*` fills from it, so the Figma file already ships Electric chips on white. `colors.md` §2 also allows Electric on White "as a large block, bar, or shape only". Only `SKILL.md` bans every role. A ruling here settles all three texts and the Figma variable.
